...
<body>
  <h1 id="tieude">Trang đăng nhập dành cho nhân viên<h1>
      <form class="login" action="" method="POST">
        <input type="text" name="email" placeholder="Email">
        <input type="password" name="matkhau" placeholder="Mật khẩu">
        <button type="submit" name="submit">Đăng nhập</button>
        <div><a href="<?php echo SITEURL; ?>admin/login.php">Người quản lí</a></div>
      </form>
</body>
</html>
<?php
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * FROM nguoidung WHERE madangnhap='$email' AND matkhau='$pass' AND nhanvien='yes' ";
    // var_dump($sql);
    // die();
    ...
    if($count == 1) {
        $_SESSION['user']=$email;//kiem tra neu user da dang nhap hoac neu khong logout se unset no
